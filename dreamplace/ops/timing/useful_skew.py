...
def solve_useful_skew(graph, max_skew=None):
    ...
    if max_skew is not None and float(max_skew) <= 0.0:
        register_names = [register["register_name"] for register in registers]
        return {
            "success": True,
            "status": 0,
            "message": "max_skew<=0, solved as exact no-skew",
            "margin": min(
                [float(edge["setup_slack"]) for edge in edges if _is_number(edge.get("setup_slack"))] or [0.0]
            ),
            "skews": {name: 0.0 for name in register_names},
            "num_constraints": 0,
            "num_registers": len(register_names),
            "num_edges": len(edges),
            "max_skew": max_skew,
            "solver": "exact-no-skew",
        }
